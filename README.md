...
这份仓库是面向 GitHub 公开整理过的版本，已经去掉了本地运行数据和内部规划文档；同时保留了 Windows 一键启动所需的 FFmpeg 目录。
...
如果你是 Windows 用户，并且机器上已经安装了 Python 3.10+ 和 Node.js，可以直接：
...
- `tools/ffmpeg/` 下的 Windows FFmpeg 二进制，供 `start-win.bat` 和本地 AI 媒体流程直接使用

这不影响代码结构，但如果你要使用 AI 素材分析、音频转写、自动文案生成，建议自行准备：

1. 正常安装 Python 依赖中的 `faster-whisper`
2. 在“关于”页或 `gui_config.json` 中配置 AI 提供商、API Base、API Key、默认模型
