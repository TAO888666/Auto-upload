...
async def _open_douyin_scan_login_tab(page: Page) -> None:
    scan_login_tabs = [
        page.get_by_text("扫码登录", exact=True).first,
        page.get_by_text("扫码登录").first,
        page.get_by_text("二维码登录", exact=True).first,
        page.get_by_text("二维码登录").first,
    ]

    for scan_login_tab in scan_login_tabs:
        if not await scan_login_tab.count():
            continue
        try:
            await scan_login_tab.wait_for(state="visible", timeout=5000)
            await scan_login_tab.click(timeout=1000)
...
async def _find_douyin_qrcode_locator(page: Page):
    await _open_douyin_scan_login_tab(page)

    scan_login_tab = page.get_by_text("扫码登录", exact=True).first
    if await scan_login_tab.count():
        qrcode_img = (
            scan_login_tab
            .locator("..")
    ...
    qrcode_locators = [
        ...
        page.locator('img[alt*="二维码"]').first,
        page.locator('img[src^="data:image/"]').first,
        page.locator('img[src*="qrcode"]').first,
        page.locator('canvas[aria-label*="二维码"]').first,
        page.locator("canvas").first,
    ]

    for qrcode_locator in qrcode_locators:
        if not await qrcode_locator.count():
            continue
        try:
            await qrcode_locator.wait_for(state="visible", timeout=5000)
            return qrcode_locator
        except Exception:
            continue

    raise RuntimeError("未找到抖音登录二维码图片")


async def _extract_douyin_qrcode_src(page: Page) -> str:
    qrcode_locator = await _find_douyin_qrcode_locator(page)
    src = await qrcode_locator.get_attribute("src")
    if not src:
...
async def _save_douyin_qrcode(page: Page, account_file: str, previous_qrcode_path: Path | None = None, qrcode_callback=None) -> dict:
    qrcode_locator = await _find_douyin_qrcode_locator(page)
    qrcode_src = await qrcode_locator.get_attribute("src") or ""
    qrcode_path = build_login_qrcode_path(account_file, suffix="douyin_login_qrcode")
    ...
    if qrcode_src.startswith("data:image/"):
        ...
        await qrcode_locator.screenshot(path=str(qrcode_path))

    if previous_qrcode_path and previous_qrcode_path != qrcode_path and remove_qrcode_file(previous_qrcode_path):
        douyin_logger.info(_msg("🧹", f"临时二维码文件已清理: {previous_qrcode_path}"))

    douyin_logger.info(_msg("📷", f"二维码已准备好，保存到: {qrcode_path}"))
    qrcode_content = decode_qrcode_from_path(qrcode_path)
    if qrcode_content:
        print_terminal_qrcode(qrcode_content, qrcode_path, "抖音APP")
    else:
        douyin_logger.warning(_msg("😶", f"终端无法完整显示二维码，请打开 {qrcode_path} 扫码"))
...
async def _wait_for_douyin_login(
    ...
    qrcode_path = Path(qrcode_info["image_path"]) if qrcode_info and qrcode_info.get("image_path") else None
    checks = 0

    while max_checks is None or checks < max_checks:
        if await _is_douyin_login_completed(page):
            douyin_logger.info(_msg("✅", f"登录完成，当前页面: {page.url}"))
            return _build_login_result(True, "success", "抖音登录成功", account_file, qrcode_info, page.url)

        if context is not None:
            await context.storage_state(path=account_file)
            if await cookie_auth(account_file):
                douyin_logger.info(_msg("✅", f"cookie 校验通过，当前页面: {page.url}"))
                return _build_login_result(True, "success", "抖音登录成功", account_file, qrcode_info, page.url)
        ...
        if qrcode_info is not None:
            ...
            if await expired_box.count() and await expired_box.is_visible():
                douyin_logger.warning(_msg("😶", "二维码已失效，正在刷新"))
    ...
    async with async_playwright() as playwright:
        ...
        try:
            ...
            if headless:
                ...
                douyin_logger.info(_msg("🌐", "浏览器已打开，请在浏览器中扫码并完成登录，程序会一直等待直到登录成功"))
            ...
            result = await _wait_for_douyin_login(
                ...
                max_checks=max_checks if headless else None,
            )
...
class DouYinBaseUploader(BaseVideoUploader):
    def __init__(
        ...
        self.self_declaration = bool(self_declaration)
    ...
    async def apply_self_declaration(self, page: Page) -> None:
        if not self.self_declaration:
            return

        trigger = page.get_by_text("请选择自主声明", exact=True).first
        if not await trigger.count():
            douyin_logger.info(_msg("🧍", "当前页面没有出现自主声明入口"))
            return

        await trigger.click()
        await page.wait_for_timeout(300)

        await self.select_ai_self_declaration_from_visible_modal(page)
        douyin_logger.info(_msg("📢", "自主声明已设置为“内容由AI生成”"))

    async def select_ai_self_declaration_from_visible_modal(self, page: Page) -> None:
        dialog = page.locator('div.semi-modal-content:visible').first
        await dialog.wait_for(state="visible", timeout=5000)

        option = dialog.locator('label.semi-radio:has-text("内容由AI生成")').first
        await option.wait_for(state="visible", timeout=5000)
        await option.click()

        confirm_button = dialog.locator('button:has-text("确定")').first
        await confirm_button.wait_for(state="visible", timeout=5000)
        await confirm_button.click()

        await page.wait_for_timeout(500)

    async def handle_missing_self_declaration_modal(self, page: Page) -> bool:
        missing_title = page.locator('h5.semi-modal-title:has-text("未添加自主声明")').first
        if not await missing_title.count():
            return False

        dialog = page.locator('div.semi-modal-content:visible').first
        await dialog.wait_for(state="visible", timeout=5000)

        add_button = dialog.locator('button:has-text("添加声明")').first
        if not await add_button.count():
            douyin_logger.warning(_msg("😵", "检测到未添加自主声明弹窗，但没找到“添加声明”按钮"))
            return False
        ...
        await self.select_ai_self_declaration_from_visible_modal(page)
        douyin_logger.info(_msg("📢", "检测到未添加自主声明，已自动补充“内容由AI生成”"))
        return True
    ...
    async def upload_video_content(self, page: Page) -> None:
        ...
        while True:
            try:
                ...
                if await self.handle_missing_self_declaration_modal(page):
                    douyin_logger.info(_msg("🏃", "自主声明已补充，小人继续尝试发布视频"))
                    await asyncio.sleep(0.5)
                    continue
    ...
    async def upload_note_content(self, page: Page) -> None:
        ...
        while True:
            try:
                ...
                if await self.handle_missing_self_declaration_modal(page):
                    douyin_logger.info(_msg("🏃", "自主声明已补充，小人继续尝试发布图文"))
                    await asyncio.sleep(0.5)
                    continue
                await self.apply_self_declaration(page)
