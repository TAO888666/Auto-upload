...
async def cookie_auth_douyin(account_file):
    try:
        profile = await fetch_account_profile_from_cookie(3, Path(account_file))
        if profile.get("isValid"):
            douyin_logger.success("[+] cookie 有效")
            return True
    except Exception as exc:
        douyin_logger.error(f"[+] 抖音接口检测失败: {exc}")

    douyin_logger.error("[+] cookie 失效，需要扫码登录")
    return False


async def _cookie_auth_by_profile_api(account_type, account_file, logger):
    try:
        profile = await fetch_account_profile_from_cookie(account_type, Path(account_file))
        if profile.get("isValid"):
            logger.success("[+] cookie valid")
            return True
    except Exception as exc:
        logger.error(f"[+] profile api check failed: {exc}")

    logger.error("[+] cookie invalid")
    return False


async def cookie_auth_xhs(account_file):
    return await _cookie_auth_by_profile_api(1, account_file, xiaohongshu_logger)


async def cookie_auth_tencent(account_file):
    return await _cookie_auth_by_profile_api(2, account_file, tencent_logger)


async def cookie_auth_ks(account_file):
    return await _cookie_auth_by_profile_api(4, account_file, kuaishou_logger)
