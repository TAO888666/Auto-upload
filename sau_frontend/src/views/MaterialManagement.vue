      ...
      <div class="material-search">
        ...
        <div class="action-buttons">
          <el-button
            type="danger"
            plain
            :disabled="appStore.materials.length === 0"
...
const handleDeleteAll = () => {
  const total = appStore.materials.length
  if (total === 0) {
    ElMessage.info('暂无素材可删除')
    return
  }

  ElMessageBox.confirm(
    `确定要删除全部 ${total} 个素材吗？该操作会同时删除素材文件，无法恢复。`,
    '全部删除素材',
    {
    ...
    .then(async () => {
      ...
      try {
        const response = await materialApi.deleteAllMaterials()

        if (response.code === 200) {
          appStore.setMaterials([])
          const deletedRecords = response.data?.deletedRecords ?? total
          ElMessage.success(`已删除 ${deletedRecords} 个素材`)
        } else {
          ElMessage.error(response.msg || '全部删除失败')
        }
      } catch (error) {
