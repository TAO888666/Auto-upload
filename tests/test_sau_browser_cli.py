...
class BrowserCliParserTests(unittest.TestCase):
    ...
    def test_douyin_upload_note_accepts_self_declaration_flag(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        self.assertTrue(args.self_declaration)
    ...
    def test_weixin_upload_video_accepts_draft_flag(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            video_path = Path(tmp_dir) / "demo.mp4"
            video_path.write_bytes(b"video")
...
class BrowserCliDispatchTests(unittest.TestCase):
    ...
    def test_dispatch_weixin_upload_video_passes_draft_flag(self):
        args = Namespace(
            platform="weixin",
            action="upload-video",
            account="creator",
        ...
        with patch("sau_cli.upload_weixin_video", new=AsyncMock()) as mock_upload:
            asyncio.run(sau_cli.dispatch(args))

        request = mock_upload.await_args.args[0]
        self.assertEqual(request.title, "视频号标题")
        self.assertEqual(request.description, "视频号简介")
        self.assertTrue(request.is_draft)

    def test_dispatch_douyin_upload_note_passes_self_declaration_flag(self):
        args = Namespace(
            platform="douyin",
            action="upload-note",
            account="creator",
        ...
        request = mock_upload.await_args.args[0]
        self.assertTrue(request.self_declaration)
