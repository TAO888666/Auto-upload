...
from uploader.tencent_uploader.main import (
    TENCENT_PUBLISH_STRATEGY_IMMEDIATE as WEIXIN_PUBLISH_STRATEGY_IMMEDIATE,
    TENCENT_PUBLISH_STRATEGY_SCHEDULED as WEIXIN_PUBLISH_STRATEGY_SCHEDULED,
    TencentVideo as WeiXinVideo,
    TencentNote as WeiXinNote,
    cookie_auth as weixin_cookie_auth,
    tencent_setup as weixin_setup,
)
...
@dataclass(slots=True)
class WeixinVideoUploadRequest:
    account_name: str
    video_file: Path
    title: str
    description: str
    ...
    thumbnail_file: Path | None = None
    is_draft: bool = False
    publish_strategy: str = WEIXIN_PUBLISH_STRATEGY_IMMEDIATE
    debug: bool = True
    headless: bool = True
...
async def login_weixin_account(account_name: str, headless: bool = True) -> dict:
    account_file = resolve_account_file("weixin", account_name)
    return await weixin_setup(str(account_file), handle=True, return_detail=True, headless=headless)


async def check_weixin_account(account_name: str) -> bool:
    account_file = resolve_account_file("weixin", account_name)
    if not account_file.exists():
        return False
    return await weixin_cookie_auth(str(account_file))


async def upload_weixin_video(request: WeixinVideoUploadRequest) -> Path:
    account_file = resolve_account_file("weixin", request.account_name)
    is_ready = await weixin_setup(str(account_file), handle=False)
    if not is_ready:
        raise RuntimeError(
            f"Weixin cookie is missing or expired: {account_file}. Run `sau weixin login --account {request.account_name}` first."
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    upload_video_parser.add_argument("--self-declaration", action="store_true", help="Apply Douyin self declaration as AI-generated content")
    ...
    upload_note_parser.add_argument("--self-declaration", action="store_true", help="Apply Douyin self declaration as AI-generated content")
    ...
    weixin_parser = platform_parsers.add_parser("weixin", help="Weixin (视频号) operations")
    weixin_actions = weixin_parser.add_subparsers(dest="action", required=True)

    for action_name in ("login", "check"):
        action_parser = weixin_actions.add_parser(action_name, help=f"Weixin {action_name}")
        action_parser.add_argument("--account", required=True, help="Weixin user-defined account_name")
        if action_name == "login":
            add_runtime_flags(action_parser)

    weixin_upload_video_parser = weixin_actions.add_parser("upload-video", help="Upload one video to Weixin")
    weixin_upload_video_parser.add_argument("--account", required=True, help="Weixin user-defined account_name")
    weixin_upload_video_parser.add_argument("--file", required=True, type=existing_file_path, help="Video file path")
    weixin_upload_video_parser.add_argument("--title", required=True, help="Video title")
    weixin_upload_video_parser.add_argument("--desc", default="", help="Optional video description")
    weixin_upload_video_parser.add_argument("--tags", default="", help="Comma-separated tags, such as tag1,tag2")
    weixin_upload_video_parser.add_argument("--schedule", type=schedule_value, help=f"Schedule time in {schedule_help}")
    weixin_upload_video_parser.add_argument("--thumbnail", type=existing_file_path, help="Optional thumbnail path")
    weixin_upload_video_parser.add_argument("--draft", action="store_true", help="Save as draft instead of direct publish")
    add_runtime_flags(weixin_upload_video_parser)
    return parser
...
async def dispatch(args: argparse.Namespace) -> int:
    ...
    if args.platform == "weixin":
        if args.action == "login":
            result = await login_weixin_account(args.account, headless=args.headless)
            if not result["success"]:
                raise RuntimeError(result["message"])
            print(f"Weixin login flow completed: {result['account_file']}")
            return 0

        if args.action == "check":
            is_valid = await check_weixin_account(args.account)
            print("valid" if is_valid else "invalid")
            return 0 if is_valid else 1

        publish_strategy = WEIXIN_PUBLISH_STRATEGY_SCHEDULED if args.schedule else WEIXIN_PUBLISH_STRATEGY_IMMEDIATE

        if args.action == "upload-video":
            request = WeixinVideoUploadRequest(
                account_name=args.account,
                video_file=args.file,
                title=args.title,
                description=args.desc,
            ...
            await upload_weixin_video(request)
            print(f"Weixin video upload submitted: {request.video_file}")
            return 0

        raise RuntimeError(f"Unsupported Weixin action: {args.action}")
